...
# # s_call_least_abs_distance_regression [<img src="https://www.arpm.co/lab/icons/icon_permalink.png" width=30 height=30 style="display: inline;">](https://www.arpm.co/lab/redirect.php?code=s_call_least_abs_distance_regression&codeLang=Python)
# For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=s_call_least_abs_distance_regression).
...
# ## [Input parameters](https://www.arpm.co/lab/redirect.php?permalink=s_call_least_abs_distance_regression-parameters)

l_ = 10  # number of leaves in CART

# ## [Step 0](https://www.arpm.co/lab/redirect.php?permalink=s_call_least_abs_distance_regression-implementation-step00): Upload data
...
# ## [Step 1](https://www.arpm.co/lab/redirect.php?permalink=s_call_least_abs_distance_regression-implementation-step01): Compute return scenarios
...
# ## [Step 2](https://www.arpm.co/lab/redirect.php?permalink=s_call_least_abs_distance_regression-implementation-step02): Compute optimal least absolute distance hedge

# +
# kernel bandwidth
i_ = r_sandp_grid.shape[0]
h = (r_sandp_grid[-1] - r_sandp_grid[0])/(i_*2)
# smoothing parameter
gamma = 2

# compute least absolute distance regression predictor on the grid
med_r_call_given_rsandp = np.zeros(i_)
for i, r_sandp_grid_i in enumerate(r_sandp_grid):
    ...
    med_r_call_given_rsandp[i] = quantile_sp(np.array([0.5]),
                                           r_call, p_given_rsandp_i,
                                           method='kernel_smoothing', h=h)

# least absolute distance regression predictor as piecewise linear interpolation
def chi(z):
    return np.interp(z, r_sandp_grid, med_r_call_given_rsandp)

r_bar_call_opt = chi(r_sandp)  # hedge returns
x_minus_xbar_opt = r_call-r_bar_call_opt  # hedged returns (residuals)
# -

# ## [Step 3](https://www.arpm.co/lab/redirect.php?permalink=s_call_least_abs_distance_regression-implementation-step03): Compute linear least absolute distance hedge 

# +
df = pd.DataFrame({'outputs': r_call, 'inputs': r_sandp})
regression = smf.quantreg('outputs ~ inputs-1', df).fit(q=0.5)
del df

# parameters of linear least absolute distance regression predictor
beta = regression.params['inputs']

# linear least absolute distance regression predictor 
def chi_beta(z):
...
# ## [Step 4](https://www.arpm.co/lab/redirect.php?permalink=s_call_least_abs_distance_regression-implementation-step04): Compute CART least absolute distance hedge 

# +
# fit CART least absolute distance regression
tree_delta_star = DecisionTreeRegressor(criterion='mae', max_leaf_nodes=l_) \
...
def plot_call_returns(title, predictor, pred_label, legend_label, r_bar_call, u, method=None):
    ...
    # plot hedge vs hedged returns (residual)
...
fig_condq = plot_call_returns(title = 'Least absolute distance regression',
                              predictor = chi,
                              pred_label = r'$\chi({R}^{\mathit{S\&P}})$',
                              legend_label = 'Cond. median',
                              r_bar_call = r_bar_call_opt,
                              u = x_minus_xbar_opt)

# figure for linear prediction
fig_linq = plot_call_returns(title = 'Linear least absolute distance regression',
                             predictor = chi_beta,
                             pred_label = r'$\chi_{\beta}({R}^{\mathit{S\&P}})$',
                             legend_label = 'LAD lin. approx.',
                             r_bar_call = r_bar_call_beta,
                             u = x_minus_xbar_beta)

# figure for CART prediction
fig_cart = plot_call_returns(title = 'CART least absolute distance regression',
                             predictor = chi_cart,
                             pred_label = r'$\chi_{\beta, \Delta^{\ast}}({R}^{\mathit{S\&P}})$',
                             legend_label = 'LAD CART approx.',
