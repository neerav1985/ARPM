...
def simulate_t(mu, sigma2, nu, j_, stoc_rep=None, method='PCA', d=None):
    """For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=eb-ellsimulationimpl).
    ...
    if stoc_rep is False:
        ...
        r = np.sqrt(n_*stats.f.ppf(np.random.rand(j_, 1), n_, nu))
        ...
        v = stats.chi2.ppf(np.random.rand(j_, 1), nu)
