...
def fit_lfm_mlfp(x, z, p=None, nu=4, tol=1e-3, fit_intercept=True,
                 maxiter=500, print_iter=False, rescale=False, shrink=False,
                 lam=0.):
    """For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=MLFPEstimRegLoadDispFun).
    ...
    if shrink:
        alpha, beta, sigma2, u = fit_lfm_lasso(x, z, p,
                                               lam=lam,
    ...
    for i in range(maxiter):
        ...
        if shrink:
            alpha, beta, sigma2, u = fit_lfm_lasso(x, z, q,
                                                   lam=lam,
                                                   fit_intercept=fit_intercept)
        else:
