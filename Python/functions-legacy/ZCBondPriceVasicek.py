...
def ZCBondPriceVasicek(tau,theta):
    ...
    # For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=zcb_vasicek).
    ## Code
    
    beta = (1-exp(-theta.theta1*tau))/theta.theta1
    
    alpha = (theta.theta2-((theta.theta3**2)/(2*theta.theta1**2)))*(beta - tau)-((theta.theta3**2)*(beta**2)/(4*theta.theta1))
