...
def equation(w, y, c, sigma2, a_tilde, b_tilde,n_):
    ...
    cprime_Y = a_tilde + trace((c@sigma2).dot(pinv(eye(n_)-2*w*c@sigma2)))+b_tilde.T@((w*sigma2-(w**2)*sigma2@c@sigma2).dot(pinv((eye(sigma2.shape[0])-2*w*c@sigma2)@(eye(sigma2.shape[0])-2*w*c@sigma2))))@b_tilde
    output = cprime_Y - y
...
def cder2(w, c, sigma2, b_tilde, n_):
    cder2_Y = 2*trace(((c@sigma2)@(c@sigma2)).dot(pinv((eye(n_) - 2*w*c@sigma2)@(eye(n_) - 2*w*c@sigma2))))+\
...
def SaddlePointQuadN(y, a, b, c, mu, sigma2, w_hat_0=0, threshold=10**-1):
    ...
    for i in range(leny):
        if n_ == 1:  # solve the quadratic equation analitically
            ...
            if test_cder2 >= 0:
                ...
                x, info, _, _ = fsolve(lambda w_hat: equation(w_hat,y[0,i],c,sigma2,a_tilde,b_tilde, n_), w_hat_0, full_output=True)
    ...
    for i in range(leny):
        if np.isnan(w_hat[0,i]):
            ...
            c_Y[0,i] = w_hat[0,i]*a_tilde - 0.5*log(det(eye(n_) - 2*w_hat[0,i]*sigma2@c)) + 0.5*(w_hat[0,i]**2)*b_tilde.T@aux@b_tilde
