...
def FitHeston(tau,k,sigma_impl,r,s_0,z_0):
    ...
    # For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=fit_heston).
    ...
    #optimization
    res = least_squares(objective,z0_tilde,args=(tau,k,sigma_impl,r,s_0),bounds=(lb,ub),max_nfev=100, ftol=1e-6, verbose=0)
    x = res.x
    ...
    for i in range(len(tau)):
        for j in range(k.shape[1]):
            c_heston[i,j] = CallPriceHestonFFT(s_0,k[i,j],r,tau[i],r_[kappa, sigma_bar**2, lam, rho, sigma_0**2])
    return z, c_heston
...
def objective(x,tau,k,sigma_impl,r,s_0):
    ...
    for j in range(len(tau)):
        c_heston[j,:k.shape[1]] = CallPriceHestonFFT(s_0,k[j,:],r,tau[j],r_[kappa, sigma_bar**2, lam, rho, sigma_0**2])
