...
def EwmaIncludingStartingDays(Smoothing, data_in):
    # For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=ewma).
    ...
    for ir in range(Days_for_ewma,t_):
        ewma[:,ir]=(data_in[:,ir]-exp(coeff@Days_for_ewma)@data_in[:,ir+Days_for_ewma] )/sumcoeff+exp[coeff]@ewma[:,ir-1]
    return ewma
