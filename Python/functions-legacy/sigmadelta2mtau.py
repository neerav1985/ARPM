...
def sigmadelta2mtau(n_grid_option,sigma_delta,maturity_opt,delta,tau_yield_curve,yield_curve):
    ...
    max_m = npmax(tile(norm.ppf(delta)[np.newaxis,...,np.newaxis],(nmat_Option,1,t_))*sigma_delta-(y_grid_t + sigma_delta**2/2)*
                       tile(sqrt(maturity_opt)[...,np.newaxis,np.newaxis],(1,k_,t_)))*0.8
    min_m = npmin(tile(norm.ppf(delta)[np.newaxis,...,np.newaxis],(nmat_Option,1,t_))*sigma_delta-(y_grid_t + sigma_delta**2/2)*
                       tile(sqrt(maturity_opt)[...,np.newaxis,np.newaxis],(1,k_,t_)))*0.8
    ...
    for t in range(t_):
        for n in range(nmat_Option):
            implVol_t[n,:,t] = Delta2MoneynessImplVol(sigma_delta[[n],:,t],delta,maturity_opt[[n]],y_grid_t[n,:,[t]],m_grid_option)[0] #linear interpolation
